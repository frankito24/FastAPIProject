...
DOWNLOAD_DIR = "../downloads/hospital_poblacion_asignada_csvs"
...
def clean_filename(text):
    nombre = text.lower()
    nombre = nombre.replace("población asignada al ", "")
...
with webdriver.Chrome(options=chrome_options) as driver:
    ...
    for grupo in grupos:
        if click_node(grupo):
            time.sleep(3)
            # Itera por todos los nodos Poblacion asignada (pueden aparecer varios)
            poblacion_asignada_nodes = driver.find_elements(By.XPATH, "//a[contains(@href, 'javascript:cargar_temas') and contains(text(), 'Poblacion asignada')]")
            for node in poblacion_asignada_nodes:
                ...
                try:
                    ...
                    time.sleep(3)
                    # Itera por todos los nodos Población asignada al *
                    poblacion_hospital_nodes = driver.find_elements(By.XPATH, "//a[contains(@href, 'javascript:mostrar_tabla') and contains(text(), 'Población asignada al ')]")
                    for hosp_node in poblacion_hospital_nodes:
