...
DOWNLOAD_DIR = "../downloads/hospital_recursos_asignados_csvs"
...
def clean_filename(text):
    nombre = text.lower()
    nombre = nombre.replace("recursos materiales del ", "")
...
with webdriver.Chrome(options=chrome_options) as driver:
    ...
    for grupo in grupos:
        if click_node(grupo):
            time.sleep(3)
            # Itera por todos los nodos Recursos materiales (pueden aparecer varios)
            recursos_asignada_nodes = driver.find_elements(By.XPATH, "//a[contains(@href, 'javascript:cargar_temas') and contains(text(), 'Recursos materiales')]")
            for node in recursos_asignada_nodes:
                ...
                try:
                    ...
                    time.sleep(3)
                    # Itera por todos los nodos Recursos asignados al *
                    recursos_hospital_nodes = driver.find_elements(By.XPATH, "//a[contains(@href, 'javascript:mostrar_tabla') and contains(text(), 'Recursos materiales del ')]")
                    for hosp_node in recursos_hospital_nodes:
